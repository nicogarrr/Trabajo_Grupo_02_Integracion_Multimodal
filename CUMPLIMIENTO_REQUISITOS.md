...
## Ampliacion: Logica de Desambiguacion Mutua

Estado: **implementado en el integrador central**.

### Objetivo

Demostrar que cuando un canal (texto o vision) tiene un nivel de confianza (confidence score) muy bajo debido a ruido o ambiguedad, el otro canal con alta certeza corrige la intencion erronea y permite ejecutar la accion correcta.

### Arquitectura

La desambiguacion mutua esta implementada dentro del metodo `check_fusion()` de la clase `MultimodalIntegrator` en:

`IntegradorMultimodal.py`

El flujo es:

1. Se intenta la fusion directa como antes: buscar `(intencion_textual, intencion_visual)` en `VALID_FUSIONS`.
2. Si la fusion directa falla, se invoca `_try_disambiguate(text_event, visual_event)`.
3. La desambiguacion evalua dos casos:
   - **Caso 1 – Texto ambiguo, gesto claro**: si la confianza textual es menor que `LOW_TEXT_CONFIDENCE` (0.45) y la confianza visual es mayor o igual que `HIGH_VISUAL_CONFIDENCE` (0.78), el gesto corrige la intencion textual usando `VISUAL_TO_TEXT_INTENT`.
   - **Caso 2 – Gesto ambiguo, texto claro**: si la confianza visual es menor que `LOW_VISUAL_CONFIDENCE` (0.55) y la confianza textual es mayor o igual que `HIGH_TEXT_CONFIDENCE` (0.78), el texto corrige la intencion visual usando `TEXT_TO_VISUAL_INTENT`.
4. Si ninguno de los dos casos aplica (por ejemplo, ambos canales tienen baja confianza), la fusion se rechaza como antes.

### Umbrales de confianza

| Parametro | Valor | Significado |
| --- | ---: | --- |
| `LOW_TEXT_CONFIDENCE` | 0.45 | Confianza textual considerada insuficiente |
| `HIGH_TEXT_CONFIDENCE` | 0.78 | Confianza textual considerada alta y fiable |
| `LOW_VISUAL_CONFIDENCE` | 0.55 | Confianza visual considerada insuficiente |
| `HIGH_VISUAL_CONFIDENCE` | 0.78 | Confianza visual considerada alta y fiable |

### Tablas de correccion mutua
...
El canal textual (`CanalTextoChefZeroWaste.py`) calcula la confianza mediante:

- Reglas textuales: si una sola regla coincide, confianza 0.92. Si varias reglas coinciden (frase ambigua), confianza 0.34.
- Clasificador BoW: `decision_function` + softmax para obtener una probabilidad.
- Combinacion: si regla y clasificador coinciden, la confianza sube; si discrepan, se limita.

La confianza se publica en `event_text.json` dentro del campo `confidence`.

El canal visual (`vision/main.py`) obtiene la confianza directamente del score del modelo MediaPipe y la publica en `event_vision.json` dentro del campo `confidence`.

### Prueba sin webcam

La demo incluye tres escenarios de desambiguacion:

```powershell
python IntegradorMultimodal.py --demo
```

Salida de los escenarios de Ampliacion C:

```text
============================================================
DEMO AMPLIACION C: Desambiguacion Mutua
============================================================

--- Escenario 1: Texto ambiguo, gesto claro ---
  Texto PLN dice: 'lista_compra' (conf=0.30) -- frase ruidosa
  Vision dice: 'iniciar_receta' (conf=0.92) -- gesto pizca_sal claro
  Esperado: el gesto corrige el texto a 'recomendar_receta'.

DESAMBIGUACION MUTUA: canal 'text' corregido por canal 'vision'
  Intent original del text: 'lista_compra' (confianza 0.30)
  Corregido a 'recomendar_receta' gracias a 'iniciar_receta' del canal vision (confianza 0.92)

FUSION VALIDA: texto='recomendar_receta' + gesto='iniciar_receta' (800.0 ms)
ACCION FINAL: Confirmar el inicio de la receta solicitada por texto.

--- Escenario 2: Gesto ambiguo, texto claro ---
  Texto PLN dice: 'sustituir_ingrediente' (conf=0.95) -- frase clara
  Vision dice: 'aumentar_raciones' (conf=0.40) -- gesto borroso
  Esperado: el texto corrige el gesto a 'marcar_sustitucion'.

DESAMBIGUACION MUTUA: canal 'vision' corregido por canal 'text'
  Intent original del vision: 'aumentar_raciones' (confianza 0.40)
  Corregido a 'marcar_sustitucion' gracias a 'sustituir_ingrediente' del canal text (confianza 0.95)

FUSION VALIDA: texto='sustituir_ingrediente' + gesto='marcar_sustitucion' (1000.0 ms)
ACCION FINAL: Aplicar la sustitucion del ingrediente indicado por texto.
SUSTITUCION: leche -> bebida vegetal (sin_lactosa)

--- Escenario 3: Ambos canales con baja confianza ---
  Texto PLN dice: 'lista_compra' (conf=0.25)
  Vision dice: 'aumentar_raciones' (conf=0.35)
  Esperado: NO se desambigua, fusion rechazada.
Fusion ignorada: texto='lista_compra' + gesto='aumentar_raciones' no es una combinacion valida.
```

Los tres escenarios demuestran:

- Escenario 1: texto con confianza baja (0.30) corregido por gesto con confianza alta (0.92). El gesto `pizca_sal` -> `iniciar_receta` indica que la intencion real era `recomendar_receta`, no `lista_compra`.
- Escenario 2: gesto con confianza baja (0.40) corregido por texto con confianza alta (0.95). La frase clara "sustituye la leche" corrige el gesto mal clasificado.
- Escenario 3: ambos canales con confianza baja. El sistema no inventa una correccion y rechaza la fusion.

### Salida de fusion con desambiguacion

Cuando se aplica desambiguacion, el JSON de fusion incluye un campo adicional:
...
Este campo permite trazar que canal fue corregido, cual era su intencion original erronea, por que canal fue corregido y con que nivel de confianza.
