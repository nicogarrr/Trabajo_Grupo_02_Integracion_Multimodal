...
Remove-Item -LiteralPath (Join-Path $ProjectRoot "event_text.json.tmp") -Force -ErrorAction SilentlyContinue
Remove-Item -LiteralPath (Join-Path $ProjectRoot "event_vision.json.tmp") -Force -ErrorAction SilentlyContinue
Remove-Item -LiteralPath (Join-Path $ProjectRoot "event_fusion.json.tmp") -Force -ErrorAction SilentlyContinue
...
if (-not $DryRun) {
    ...
    Write-Host "1. Haz un gesto en la ventana de Vision Gestos."
    Write-Host "2. Escribe la frase en Chat PLN y pulsa Enter para aceptar la categoria."
    Write-Host "3. Mira la fusion en la ventana del Integrador."
}
