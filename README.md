...
Al ejecutarse por primera vez, el modelo de Hugging Face se descargara automaticamente en la cache local (por ejemplo en `~/.cache/huggingface/hub/`).
...
## Ampliacion Desambiguacion Mutua

Cuando un canal tiene confianza baja (ruido, frase ambigua, mano parcialmente fuera de cuadro) y el otro canal tiene confianza alta, el integrador corrige automaticamente la intencion erronea usando la certeza del canal fiable.

Umbrales configurados en `IntegradorMultimodal.py`:

- Texto con confianza < 0.45 y gesto con confianza >= 0.78: el gesto corrige el texto.
- Gesto con confianza < 0.55 y texto con confianza >= 0.78: el texto corrige el gesto.
- Ambos con confianza baja: la fusion se rechaza.
...
La seccion `DEMO AMPLIACION C` muestra tres escenarios: texto corregido por gesto, gesto corregido por texto, y rechazo cuando ambos canales son poco fiables.

## Ampliacion Head/Gaze Tracking

Esta ampliacion anade un tercer canal de percepcion pasiva usando MediaPipe Face Landmarker. Detecta la orientacion de la cabeza y mapea el foco de atencion a zonas logicas de la interfaz (izquierda=ingredientes, derecha=pasos, arriba=titulo, centro=receta).
...
Este modo abre el integrador, el canal de chat PLN y un script visual combinado que ejecuta tanto el reconocimiento de gestos como el head tracking desde la misma webcam.

Uso recomendado:
1. Mira hacia una zona de la pantalla (ej. a la izquierda).
2. Haz un gesto con la mano (ej. la cruz de sustituir_ingrediente).
3. Escribe en el chat PLN el texto correspondiente y pulsa Enter.
4. El integrador mostrara la fusion e indicara el foco de atencion espacial del usuario.

Demo sin webcam:

```powershell
python vision\gaze_head_tracking.py --demo
```
...
- `IntegradorMultimodal.py`: fusion semantica tardia y desambiguacion mutua.
...
- `vision/gaze_head_tracking.py`: ampliacion D, head tracking + gestos combinados.
- `lanzar_ampliacion_gaze.ps1`: abre integrador, head tracking y PLN.
- `models/hand_landmarker.task`: detector de mano usado para recortar la region visual antes de CLIP.
- `models/face_landmarker.task`: detector de cara usado para el head tracking.
